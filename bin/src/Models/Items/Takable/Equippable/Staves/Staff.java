...
import Models.Entities.Skills.PassiveSkills.PassiveSkillsVisitors.StaffVisitor;
...
public class Staff extends EquippableItem{
    private WeaponType weaponType = WeaponType.STAFF;
    // We will just be instantiating different "Versions" of this equipment type
    public Staff(Image image, StatModificationList mods, String name, String description, RequirementList requirements, int ID, int price) {
        this.image = image;
    ...
    public boolean equip(Equipment equipment, PassiveSkillList passiveSkillList) {
        ...
        for(int i = 0; i < passiveSkillList.size(); i++) {
            passiveSkillList.get(i).acceptVisitor(new StaffVisitor());
        }
        return true;
    }
